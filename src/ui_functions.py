...
def add_user_ui(user_role):
    ...
    try:
        username = get_validated_input(
            "Username (8-10 characters): ", validate_username
        )
        password = get_validated_input(
            "Password (12-30 characters): ", validate_password
        )
...
def reset_user_password_ui(user_role):
    ...
    try:
        ...
        if update_user_password(username, temp_password):
            ...
            print(
                "Please provide this to the user and advise them to change it immediately."
            )
        else:
...
def add_traveller_ui():
    ...
    try:
        ...
        first_name = get_validated_input("First Name: ", validate_name, "First Name")
        last_name = get_validated_input("Last Name: ", validate_name, "Last Name")
        birthday = get_validated_input(
            "Birthday (YYYY-MM-DD): ", validate_date, "Birthday"
        )
        ...
        mobile_phone = get_validated_input(
            "Mobile Phone (8 digits only): ", validate_phone_number
        )
        driving_license = get_validated_input(
            "Driving License (XXDDDDDDD or XDDDDDDDD): ", validate_driving_license
        )
...
def search_travellers_ui():
    ...
    try:
        search_term = get_validated_input(
            "Search term (name, email, or customer ID): ", validate_search_term
        )
...
def update_traveller_ui():
    ...
    try:
        search_term = get_validated_input(
            "Search for traveller to update: ", validate_search_term
        )
        results = search_travellers(search_term)
        ...
        if len(results) > 1:
            ...
            traveller_id = get_validated_input(
                "Enter traveller ID to update: ",
...
def delete_traveller_ui():
    ...
    try:
        search_term = get_validated_input(
            "Search for traveller to delete: ", validate_search_term
        )
        results = search_travellers(search_term)
        ...
        confirm = input("\nAre you sure you want to delete this traveller? (y/n): ")
...
def add_scooter_ui():
    ...
    try:
        ...
        serial_number = get_validated_input(
            "Serial Number (10-17 alphanumeric): ", validate_scooter_serial
        )
        top_speed = get_validated_input("Top Speed (km/h): ", validate_speed)
        battery_capacity = get_validated_input(
            "Battery Capacity (Wh): ", validate_battery_capacity
        )
        state_of_charge = get_validated_input(
            "State of Charge (%): ", validate_percentage, "State of Charge"
        )
        target_range_min = get_validated_input(
            "Target Range Min (%): ", validate_percentage, "Target Range Min"
        )
        target_range_max = get_validated_input(
            "Target Range Max (%): ", validate_percentage, "Target Range Max"
        )

        print("GPS Coordinates (Rotterdam region, 5 decimal places):")
        latitude = get_validated_input(
            "Latitude (51.80000-52.10000): ", validate_latitude_single
        )
        longitude = get_validated_input(
            "Longitude (4.20000-4.80000): ", validate_longitude_single
        )
        ...
        if (
            last_maintenance
            and not validate_date(last_maintenance, "Last Maintenance Date")[0]
        ):
...
def search_scooters_ui():
    ...
    try:
        search_term = get_validated_input(
            "Search term (brand, model, or serial): ", validate_search_term
        )
...
def update_scooter_ui():
    ...
    try:
        search_term = get_validated_input(
            "Search for scooter to update: ", validate_search_term
        )
        results = search_scooters(search_term)
        ...
        if len(results) > 1:
            ...
            rows = [
                [s["id"], s["brand"], s["model"], s["serial_number"]] for s in results
            ]
            print(create_display_table(headers, rows))

            scooter_id = get_validated_input(
                "Enter scooter ID to update: ", validate_positive_integer, "Scooter ID"
            )
        else:
        ...
        if current_user["role"] == "service_engineer":
            print(
                "Service Engineers can update: State of Charge, Target Range, Location, Out-of-Service Status, Mileage, Last Maintenance Date"
            )
        ...
        if (
            new_lat
            and new_lng
            and validate_latitude_single(new_lat)
            and validate_longitude_single(new_lng)[0]
        ):
            updates["latitude"] = new_lat
            updates["longitude"] = new_lng

        new_target_range_min = input(
            f"Target Range Min % [{scooter['target_range_min']}]: "
        )
        if new_target_range_min and validate_percentage(new_target_range_min)[0]:
            updates["target_range_min"] = new_target_range_min

        new_target_range_max = input(
            f"Target Range Max % [{scooter['target_range_max']}]: "
        )
        if new_target_range_max and validate_percentage(new_target_range_max)[0]:
            updates["target_range_max"] = new_target_range_max

        new_out_of_service = input(
            f"Out of Service (y/n) [{scooter['out_of_service']}]: "
        )
        if new_out_of_service in [
            "y",
            "Y",
        ]:
        ...
        new_last_maintenance = input(
            f"Last Maintenance Date (YYYY-MM-DD) [{scooter['last_maintenance_date'] if scooter['last_maintenance_date'] else 'N/A'}]: "
        )
        ...
        if current_user["role"] in ["super_admin", "system_admin"]:
            ...
            new_battery_capacity = input(
                f"Battery Capacity (Wh) [{scooter['battery_capacity']}]: "
            )
            if (
                new_battery_capacity
                and validate_battery_capacity(new_battery_capacity)[0]
            ):
                updates["battery_capacity"] = new_battery_capacity
        else:
            print(
                "You do not have permission to update brand, model, serial number, top speed, or battery capacity."
            )
            log_event(
...
def delete_scooter_ui():
    ...
    try:
        search_term = get_validated_input(
            "Search for scooter to delete: ", validate_search_term
        )
        results = search_scooters(search_term)
        ...
        if len(results) > 1:
            ...
            scooter_id = get_validated_input(
                "Enter scooter ID to delete: ", validate_positive_integer, "Scooter ID"
            )
        else:
            scooter_id = results[0]["id"]
...
def restore_backup_ui():
    ...
    try:
        ...
        for i, backup in enumerate(backups, 0):
            print(
                f"{i}. {backup['filename']} ({backup['created'][:19].replace('T', ' ')})"
            )
            backup_choices.append(str(i))

        choice = get_validated_input(
            f"Select backup ({', '.join(backup_choices)}): ",
            validate_choice,
            backup_choices,
            "Backup",
        )
...
def generate_restore_code_ui():
    ...
    try:
        ...
        for i, admin in enumerate(sys_admins, 0):
            print(
                f"{i}. {admin['username']} ({admin['first_name']} {admin['last_name']})"
            )
            admin_choices.append(str(i))

        admin_choice = get_validated_input(
            f"Select administrator ({', '.join(admin_choices)}): ",
            validate_choice,
            admin_choices,
            "Administrator",
        )
        selected_admin = sys_admins[
            int(admin_choice)
        ]  # input is validated, so cast is safe
        ...
        backup_choice = get_validated_input(
            f"Select backup ({', '.join(backup_choices)}): ",
            validate_choice,
            backup_choices,
            "Backup",
        ...
        code = generate_restore_code_for_admin(
            selected_admin["username"], selected_backup["filename"]
        )
...
def revoke_restore_code_ui():
    ...
    try:
        ...
        for i, code_info in enumerate(codes, 0):
            print(
                f"{i}. {code_info['code']} (Admin: {code_info['admin_username']}, Backup: {code_info['backup_file']})"
            )
            code_choices.append(str(i))

        choice = get_validated_input(
            f"Select code to revoke ({', '.join(code_choices)}): ",
            validate_choice,
            code_choices,
            "Code",
        )
        ...
        confirm = input(f"Revoke code {selected_code['code']}? (y/n): ")
    ...
    try:
        ...
        for log in logs:
            rows.append(
                [
                    ...
                    log["username"],
                    log["description"][:50]
                    + ("..." if len(log["description"]) > 50 else ""),
        ...
        if logs:
            ...
            if choice and choice.isdigit():
                log_num = choice
                selected_log = next(
                    (log for log in logs if log["log_number"] == log_num), None
                )
                if selected_log:
                    ...
                    print(
                        f"Suspicious: {'Yes' if selected_log['suspicious'] else 'No'}"
                    )
                else:
