...
def get_user_by_username(username):
    try:
        ...
        with closing(get_db_connection()) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT * FROM users WHERE username_hash = ?", (username_hash,)
            )
            row = cursor.fetchone()
...
def delete_user(username):
    try:
        ...
        with closing(get_db_connection()) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "DELETE FROM users WHERE username_hash = ?", (username_hash,)
            )
            conn.commit()
...
def update_user_password(username, new_password):
    try:
        ...
        with closing(get_db_connection()) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "UPDATE users SET password_hash = ? WHERE username_hash = ?",
        ...
        with closing(get_db_connection()) as conn:
            ...
            """,
                (
                    ...
                    encrypted_data["driving_license"],
                    encrypted_reg_date,
                ),
            )
            conn.commit()
        ...
        with closing(get_db_connection()) as conn:
            ...
            for row in rows:
                ...
                if (
                    ...
                    or search_lower in email.lower()
                    or search_lower in customer_id.lower()
                    or search_lower in f"{first_name} {last_name}".lower()
                ):
                    results.append(
                        {
                            "id": row["id"],
...
def create_scooter(scooter_data):
    try:
        ...
        encrypted_data = {
            ...
            "last_maintenance_date": encrypt_data(
        ...
        with closing(get_db_connection()) as conn:
            ...
            """,
                (
                    ...
                    encrypted_data["top_speed"],
                    encrypted_data["battery_capacity"],
                    encrypted_data["state_of_charge"],
                    encrypted_data["target_range_min"],
                    encrypted_data["target_range_max"],
                    encrypted_data["latitude"],
                    encrypted_data["longitude"],
                    scooter_data.get("out_of_service", 0),
                    encrypted_data["mileage"],
    ...
    try:
        ...
        with closing(get_db_connection()) as conn:
            ...
            for row in rows:
                ...
                if (
                    search_lower in brand.lower()
                    or search_lower in model.lower()
                    or search_lower in serial_number.lower()
                    or search_lower in f"{brand} {model}".lower()
                ):
                    results.append(
                        {
                            "id": row["id"],
